#criar um botão para entrar no iniciar chat
# abrir um popup para colocar o nick e um botão para entrar no chat
# quando entrar no chat: (aparece para todo mundo)
    # colocar uma caixa de texto e um botão para enviar mensagem
# a cada mensagem que você envia (aparece para todo mundo)
    # Nome: Texto da mensagem
import flet as ft

def main(pagina):
    texto = ft.Text("Gabzap")

    chat = ft.Column(scroll=ft.ScrollMode.ALWAYS, expand=True)
    ...
    botao_inciar = ft.ElevatedButton("Iniciar chat", on_click=entrar_chat)
